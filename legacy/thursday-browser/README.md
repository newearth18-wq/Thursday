> **Legacy prototype — preserved, not part of Jupiter.**
> This is the Thursday Browser Alpha exactly as it was before the Jupiter
> monorepo was created (SET 0). It keeps its own `package.json`, lockfile and
> CI job, and still passes its 32 acceptance checks. Jupiter does not import
> any of it. Later SETs may port pieces (for example the provider adapters or
> the out-of-process plugin host) into Jupiter packages, rewritten against the
> Jupiter contracts — see [ADR 0001](../../docs/decisions/0001-monorepo-and-legacy-thursday.md).
> Run every command below from `legacy/thursday-browser/`.
